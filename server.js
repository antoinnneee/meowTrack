...
async function runNodeTurn(repoId, nodeId, scopeSnapshot, descendants, history, userText, author, model, pendingId, root) {
...
async function handleNodeChat(req, res, node) {
  ...
  runNodeTurn(repoId, node.id, snapshot, descendants, history, text, author, model, pendingMessage.id, root).catch(() => {});
}
...
// ── Endpoint MCP distant (POST/GET/DELETE /mcp) ──────────────────────────────
// Expose le MÊME jeu d'outils que mcp.js (module partagé mcp-tools.js), mais via
// le transport Streamable HTTP du SDK — pour un client configuré en `type: http`
// (ex. .mcp.json racine de meownopoly) sans relais local. Les outils tapent l'API
// REST du serveur lui-même en loopback (avec le token interne) : une seule source
// de vérité, aucun accès direct à db.js depuis le module d'outils.
const MCP_DEFAULT_REPO = (process.env.MEOWTRACK_DEFAULT_REPO || "").trim();
